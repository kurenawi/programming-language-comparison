...
func isOverdue(t Task) bool {
  return t.DueDate != nil && dateRe.MatchString(*t.DueDate) && *t.DueDate < today && t.Status != "done"
}

func summary(source []Task) map[string]int {
  out := map[string]int{"total_tasks": len(source), "overdue_tasks": 0, "status_todo": 0, "status_in_progress": 0, "status_done": 0, "status_blocked": 0, "points_high": 0, "points_medium": 0, "points_low": 0}
  for _, t := range source {
    if isOverdue(t) { out["overdue_tasks"]++ }
    switch t.Status {
...
func main() {
  ...
  http.HandleFunc("/tasks", func(w http.ResponseWriter, r *http.Request) {
    if r.Method == http.MethodGet {
      send(w, 200, map[string]any{"tasks": tasks})
      return
    }
    ...
    if payload.DueDate != nil && !dateRe.MatchString(*payload.DueDate) { errors = append(errors, "due_date must be YYYY-MM-DD") }
  ...
  if err := http.ListenAndServe("127.0.0.1:8123", nil); err != nil { panic(err) }
}
