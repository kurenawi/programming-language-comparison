...
func summary(source []Task) map[string]int {
  out := map[string]int{"total_tasks": len(source), "status_todo": 0, "status_in_progress": 0, "status_done": 0, "points_high": 0, "points_medium": 0, "points_low": 0}
...
func main() {
  ...
  http.HandleFunc("/tasks", func(w http.ResponseWriter, r *http.Request) {
    if r.Method == http.MethodGet { send(w, 200, map[string]any{"tasks": tasks}); return }
    ...
    if payload.Status != "todo" && payload.Status != "in_progress" && payload.Status != "done" { errors = append(errors, "status must be one of todo, in_progress, done") }
  ...
  if err := http.ListenAndServe("127.0.0.1:8013", nil); err != nil { panic(err) }
}
